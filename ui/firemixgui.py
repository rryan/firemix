...
class FireMixGUI(QtGui.QMainWindow, Ui_FireMixMain):
    ...
    def __init__(self, parent=None, app=None):
        ...
        self._mixer.default_layer().playlist()._notifier.playlist_changed.connect(
        ...
        self._mixer.default_layer().transition_starting.connect(self.transition_update_start)
    ...
    def on_transition_update_timer(self):
        p = self._mixer.default_layer().transition_progress
    ...
    def update_mixer(self):
        ...
        self.setWindowTitle("FireMix - %s - %0.2f FPS" % (self._mixer.default_layer()._playlist.name, fps))

        active_preset = self._mixer.default_layer()._playlist.get_active_preset()
        if active_preset:
            # Update wibblers
    ...
    def on_btn_next_preset(self):
        self._mixer.default_layer().next()
        self.update_playlist()

    def on_btn_prev_preset(self):
        self._mixer.default_layer().prev()
        self.update_playlist()

    def on_btn_reset_preset(self):
        paused = self._mixer.is_paused()
        self._mixer.pause()
        self._mixer.default_layer()._playlist.get_active_preset()._reset()
        self._mixer.pause(paused)
    ...
    def on_btn_add_preset(self):
        ...
        if dlg.result() == QtGui.QDialog.Accepted:
            ...
            self._mixer.default_layer()._playlist.add_preset(classname, name)

    def on_btn_remove_preset(self):
        ci = self.lst_presets.currentItem()
        if ci is not None:
            self._mixer.default_layer()._playlist.remove_preset(ci.text())
    ...
    def on_btn_clone_preset(self):
        ...
        self._mixer.default_layer()._playlist.clone_preset(old_name)
        self.update_playlist()
    ...
    def on_btn_clear_playlist(self):
        ...
        if ret == QtGui.QMessageBox.Yes:
            self._mixer.default_layer()._playlist.clear_playlist()
            self.update_playlist()

    def update_mixer_settings(self):
        self.edit_preset_duration.setValue(self._mixer.default_layer().get_preset_duration())
        self.edit_transition_duration.setValue(self._mixer.default_layer().get_transition_duration())
    ...
    def on_slider_dimmer(self):
        ...
        self._mixer.set_global_dimmer(dval)
        self.lbl_dimmer.setText("Dimmer [%0.2f]" % dval)

    def on_slider_speed(self):
        sval = round(self.slider_speed.value() / 100.0, 1)
        self._mixer.set_global_speed(sval)
        self.lbl_speed.setText("Speed [%0.1fx]" % sval)

    def update_playlist(self):
        self.lst_presets.clear()
        presets = self._mixer.default_layer()._playlist.get()
        current = self._mixer.default_layer()._playlist.get_active_preset()
        next = self._mixer.default_layer()._playlist.get_next_preset()
    ...
    def on_playlist_reorder(self):
        ...
        self._mixer.default_layer()._playlist.reorder_playlist_by_names(names)

    def on_file_load_scene(self):
        pass

    def on_file_reload_presets(self):
        self._mixer.freeze(True)
        self._mixer.default_layer()._playlist.reload_presets()
        self._mixer.freeze(False)
    ...
    def start_rename(self):
        ...
        if ok and new_name:
            if not self._mixer.default_layer()._playlist.preset_name_exists(new_name):
                self._mixer.default_layer()._playlist.rename_preset(old_name, new_name)

    def on_preset_name_changed(self, item):
        pass

    def on_preset_double_clicked(self, preset_item):
        self._mixer.default_layer().start_transition(preset_item.text())
        self.update_playlist()
        self.load_preset_parameters_table()

    def on_preset_duration_changed(self):
        nd = self.edit_preset_duration.value()
        if self._mixer.default_layer().set_preset_duration(nd):
            self._app.settings['mixer']['preset-duration'] = nd
        self.edit_preset_duration.setValue(self._mixer.default_layer().get_preset_duration())

    def on_transition_duration_changed(self):
        nd = self.edit_transition_duration.value()
        if self._mixer.default_layer().set_transition_duration(nd):
            self._app.settings['mixer']['transition-duration'] = nd
        self.edit_transition_duration.setValue(self._mixer.default_layer().get_transition_duration())

    def on_transition_mode_changed(self):
        if self._mixer.default_layer().set_transition_mode(self.cb_transition_mode.currentText()):
    ...
    def load_preset_parameters_table(self):
        ...
        if self._mixer.default_layer()._playlist.get_active_preset() == None:
            return

        parameters = self._mixer.default_layer()._playlist.get_active_preset().get_parameters()
    ...
    def on_preset_parameter_changed(self, item):
        ...
        par = self._mixer.default_layer()._playlist.get_active_preset().parameter(key.text())
        try:
    ...
    def on_file_open_playlist(self):
        paused = self._mixer.is_paused()
        self._mixer.stop()
        playlist = self._mixer.default_layer()._playlist
    ...
    def on_file_save_playlist(self):
        self._mixer.default_layer()._playlist.save()
    ...
    def on_file_generate_default_playlist(self):
        ...
        ret = dlg.exec_()
        if ret == QtGui.QMessageBox.Yes:
            self._mixer.default_layer()._playlist.generate_default_playlist()
            self.update_playlist()
    ...
    def on_btn_shuffle_playlist(self):
        ...
        self._mixer.default_layer()._playlist.shuffle_mode(shuffle)
